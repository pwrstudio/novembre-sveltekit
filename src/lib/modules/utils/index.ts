...
export const sortBy = <T>(items: T[] | null | undefined, iteratee?: (item: T) => unknown): T[] => {
    if (!items) return []
...
export const truncate = (
    str: string,
    { length = 30, separator, omission = '...' }: { length?: number; separator?: string | RegExp; omission?: string } = {},
): string => {
