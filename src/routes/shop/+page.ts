...
export async function load() {
  ...
  const sanityPosts: Product[] | undefined = productList.products?.map((p) => ({
    ...p.product,
    listingSize: p.size,
  }))
  ...
  try {
    ...
    // Fetch Shopify data
    const shopifyProducts: Client.Product[] =
      await shopifyClient.product.fetchAll()

    // Convert Shopify products to serializable format
    const serializedShopifyProducts = shopifyProducts.map((product) =>
...
function mergePosts(
  sanityPosts: Product[],
  shopifyProducts: Client.Product[],
): MergedProduct[] {
