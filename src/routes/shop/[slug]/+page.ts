...
export async function load({ params }): Promise<{ post: MergedProduct }> {
  // Fetch Sanity data
  const sanityProduct: Product = await loadData(queries.shopSingle, {
    slug: params.slug,
  })
  ...
  try {
    ...
    const shopifyProduct: Client.Product = await shopifyClient.product.fetch(
      `gid://shopify/Product/${sanityProduct.shopifyId}`,
    )
